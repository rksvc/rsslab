...
var vm = new Vue({
  ...
  methods: {
    ...
    transformFeedLink: function(feedLink) {
      return app.rsshubPath && feedLink.startsWith('rsshub:')
        ? `${app.rsshubPath}/${feedLink.slice('rsshub:'.length)}`
        : feedLink
    },
    collapseTitle: function(title) {
      const limit = 100;
    ...
    editFeedLink: function(feed) {
      var newFeedLink = prompt('Enter new feed link', feed.feed_link)
      if (newFeedLink) {
        api.feeds.update(feed.id, {feed_link: newFeedLink}).then(function() {
          feed.feed_link = newFeedLink
        })
      }
    },
    ...
    createFeed: function(event) {
      ...
      api.feeds.create(data).then(function(data) {
        vm.refreshFeeds()
        vm.refreshStats()
        vm.settings = ''
        vm.feedSelected = 'feed:' + data.id
      }).finally(function () {
    ...
    fetchFeed: function(id) {
      if (this.loading.feeds) return
      api.feeds.refresh(id).then(function() {
        vm.refreshStats()
      })
    },
    fetchFolder: function(id) {
      if (this.loading.feeds) return
      api.folders.refresh(id).then(function() {
        vm.refreshStats()
      })
    },
    fetchFeedsWithErrors: function() {
      if (this.loading.feeds) return
      api.feeds.refresh_errors().then(function() {
        vm.refreshStats()
      })
    },
